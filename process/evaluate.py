...
def Cal_eval_index(epoch, pred, loss_meathod, val_target, time_slice, mean, std):
    ...
    for item in time_slice:
        ...
        if (epoch % 50 == 0) & (epoch != 0):
            np.savetxt("./results/DeepUVI/pred_result_" + str(epoch) + ".csv", pred_index.cpu(), delimiter=',')
            np.savetxt("./results/DeepUVI/true_result_" + str(epoch) + ".csv", val_target_index.cpu(), delimiter=',')
